...
class groupSplit(object):
    ...
    def __init__(self, linkMat):
        ...
        nz = np.nonzero(linkMat)
        if(len(nz) == 2):
            pairs =  list(zip(nz[0].tolist(),
                              nz[1].tolist()))
        else:
            pairs =[]
...
if __name__ == "__main__":
    ...
    for i in [10,100,200,400,1000,10000]:
        p_0 = time.time()
#   for i in [5]:
        U = np.random.uniform(low=0, high=1.0, size=(i, i))
        S = np.tril(U) + np.tril(U, -1).T 
        mask_block = np.full((i,i), 0.001)
#       LinkMat = ((S < mask_block)*1 ).tolist()
#       for j in range(i):
#           LinkMat[j][j] = 0
        LinkMat =   S < mask_block 
        for j in range(i):
            LinkMat[j][j] = False
        p_1 = time.time()
      # print(LinkMat)
        ...
        nMol = len(MolRec)
        p_3 = time.time()
        print("--- matrix ",i,nMol," %s seconds ---" % (p_1 - p_0)),
        print("--- matrix ",i,nMol," %s seconds ---" % (p_2 - p_1)),
        print("--- Matrix ",i,nMol," %s seconds ---" % (p_3 - p_2)),
